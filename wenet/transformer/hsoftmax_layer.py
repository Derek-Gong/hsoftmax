...
class HSoftmaxLayer(nn.Module):
    def __init__(
        ...
        self.register_buffer('path_mask_sign', torch.Tensor(tree.path_mask_sign))
        self.register_buffer('path_mask_bias', torch.Tensor(tree.path_mask_bias))
    ...
    def beam_search(self, att: torch.Tensor, beam_size: int) -> List[List[int]]:
        if self.pool is None:
            ...
            self.pool = ProcessPool(self.num_workers, SearchWorker, self.inner_vector, self.tree.root, beam_size)
        att = att.to('cpu')
        logps, indexs = [], []
        # start = time.time()
    ...
    def __call__(self, att, idx = None):
        ...
        while not candidates.empty():
            ...
            if cur.node.token is not None: # leaf node
                ...
                prob_left = torch.sigmoid(torch.dot(att, self.inner_vector.weight[cur.node.idx])).item()
